...
export const ChatInterface = forwardRef<ChatHandle, ChatInterfaceProps>(
  ({ config, userId, isAdmin, externalActiveId, onConvsChange }, ref) => {
    ...
    const [exampleIdx, setExampleIdx] = useState(() =>
      Math.floor(Math.random() * config.emptyStateExamples.length)
    );
    ...
    const handleSyncToResults = async () => {
      ...
      try {
        ...
        console.error("[ChatInterface] sync failed:", err);
        toast({
          title: "Sync failed",
    ...
    const createConversation = async (
      ...
      const { data: conv, error } = await supabase
        .from("ai_chat_conversations")
        .insert({ user_id: userId, title, chat_type: config.chatType })
        .select("id, title, session_id, updated_at, chat_type, synced_search_id")
        .single();
      ...
      setExampleIdx(Math.floor(Math.random() * config.emptyStateExamples.length));
    };
    ...
    const sendMessage = async (overrideContent?: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.error("[ChatInterface] n8n HTTP error:", res.status, res.statusText, errBody);
        ...
        if (
          ...
          console.error("[ChatInterface] n8n service error:", data.error ?? data.code);
        ...
        console.error("[ChatInterface] n8n fetch failed:", err);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-foreground text-sm leading-tight">
                {activeConv?.title ?? config.emptyStateTitle}
              </h2>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
                <span className="text-xs text-muted-foreground">
                  {config.emptyStateTitle} · Online
                </span>
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div className="relative">
                  <textarea
                    ...
                    onKeyDown={handleKeyDown}
                    disabled={sending}
                    placeholder={config.emptyStateExamples[exampleIdx]}
                    rows={3}
                    className={cn(
