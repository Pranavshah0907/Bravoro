...
import type { PEGridRow } from "./PeopleEnrichmentGrid";
...
interface SentItem {
  ...
  status: string;
  created_at: string;
  grid_data: PEGridRow[] | null;
}

interface PESheetsManagerProps {
  userId: string;
  currentDraftId: string | null;
  onBack: () => void;
  onLoad: (rows: PEGridRow[], meta: { draftId?: string; name: string }) => void;
...
export const PESheetsManager = ({
  ...
  const fetchSent = async () => {
    ...
    const { data } = await supabase
      ...
      .eq("user_id", userId)
      .eq("search_type", "bulk_people_enrichment")
  ...
  const handleLoadDraft = async (d: DraftItem) => {
    ...
    try {
      ...
      onLoad(data.grid_data as PEGridRow[], { draftId: d.id, name: d.name });
    } catch {
  ...
  const handleDeleteDraft = async (d: DraftItem) => {
    ...
    const { error } = await supabase.from("people_enrichment_drafts").delete().eq("id", d.id);
    if (error) {
  ...
  const SentRow = ({ s }: { s: SentItem }) => {
    ...
    const label   = s.excel_file_name || "People Enrichment";
      ...
      <div style={{ borderTop: "2px solid #edf6f6" }}>
        ...
        {loadingSent ? (
          ...
          <div className="flex flex-col items-center py-8 gap-2">
            ...
            <p className="text-[11px]" style={{ color: "#b8cccc" }}>Submitted enrichments will appear here</p>
          </div>
        ) : (
          <div>
