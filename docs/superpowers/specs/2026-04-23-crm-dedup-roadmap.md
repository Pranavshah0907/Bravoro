...
|---|---|---|---|
| **M1** | Connect flow end-to-end (no dedup yet). Schema, Vault, Settings → Integrations tab, 4 edge functions, health check cron, Pipedrive adapter with testConnection / fetchFieldMetadata / autoMapCustomFields. | `m1-connect-flow-design.md` | ✅ Shipped on `feat/crm-dedup-m1` (Tasks 1-18 complete; Task 19 final validation pending). |
| **M1.5** | User-driven field mapping UI: expanded 13-slot mapping schema (person: firstName, lastName, email, mobilePhone, directPhone, jobTitle, linkedin, website; org: name, domain, website, linkedin, industry); "Field mapping" accordion on ConnectedCard with Edit modal; `crm-update-mapping` edge function; data migration from M1 narrow shape. Prepares for M2 dedup + M1.6 push. | `crm-field-mapping-ui-design.md` | Design drafted 2026-04-23. Plan pending. |
| **M1.6** *(NEW 2026-05-06)* | Push to CRM: user clicks "Push to CRM" on Results, picks a destination (pipeline + first stage), Bravoro creates Deal + linked Person + Org in Pipedrive. Idempotent via `crm_pushes` table. Source attribution (Quellenherkunft / ID der Ursprungsquelle / Quellkanal). Owner defaults to pushing user's matched Pipedrive user. Consumes M1.5 mappings; degrades gracefully without them. | `2026-05-06-crm-push-to-crm-design.md` | Approved 2026-05-06. Plan pending. |
| **M1.7** *(was M1.6, renumbered 2026-05-06)* | Global admin manage-any-workspace-integration from Admin panel. | (not yet written) | Deferred polish. |
| **M2** *(reshaped 2026-05-06)* | CRM contact mirror + dedup API: synced `crm_contacts` table per integration; n8n calls `crm-dedup-check` edge fn before enrichment; binary unique/duplicate verdict with email-exact / emails-array / fuzzy-name+domain match layers. Replaces the live-CRM-call mechanism in the original M2 draft. | `2026-05-06-crm-contact-mirror-and-dedup-api-design.md` (live); `2026-04-23-crm-dedup-m2-dedup-gate-design.md` (deprecated) | Approved 2026-05-06. Plan pending. |
| **M3** | Push-back enrichment writes to existing CRM rows (silent automation, distinct from M1.6's user-triggered push). When the M2 dedup verdict expands to `enrich_link_to_org`, post-enrichment write enriched data back to user-mapped columns on existing Persons/Orgs. Consumes M1.5 expanded mapping (all 13 slots). | (not yet written) | Deferred. |
| **v1.1** | HubSpot adapter + OAuth. Salesforce adapter + OAuth. Post-enrichment email-exact dedup. "Coming soon" labels in CRM dropdown. Per-destination field mapping overrides UI. Per-destination owner mapping. Reconciliation cron for mirror deletes. | (not yet written) | Deferred. |
| **v1.2** | Zoho, Close adapters. Dedup analytics dashboard. Bulk dedup API. Pipedrive Deal status sync back into Bravoro. | (not yet written) | Deferred. |
| **Speculative** | AI-powered custom field mapping (LLM classifies fields when keyword match fails). Merge.dev fallback. Multi-CRM destination splitting on push. | (not committed) | Deferred. |
